...
class SKOSQuoteLibEvent():
    ...
    def OnOverseaProductsDetail(self, bstrValue):
        msg = "【OnOverseaProductsDetail】" + bstrValue
        richTextBoxMessage.insert('end', msg + "\n")
        richTextBoxMessage.see('end')
    def OnNotifyQuoteLONG(self, nIndex):
        pSKStock = sk.SKFOREIGNLONG()
        pSKStock, nCode = m_pSKOSQuote.SKOSQuoteLib_GetStockByIndexLONG(nIndex, pSKStock)
        msg = ("【OnNotifyQuoteLONG】" + " 報價小數位數" + str(pSKStock.sDecimal) + 
               " 分母" + str(pSKStock.nDenominator) + 
               " 市場代碼" + str(pSKStock.bstrMarketNo) + 
               " 交易所代號" + str(pSKStock.bstrExchangeNo) + 
               " 交易所名稱" + str(pSKStock.bstrExchangeName) + 
               " 商品代號" + str(pSKStock.bstrStockNo) + 
               " 商品名稱" + str(pSKStock.bstrStockName) + 
               " CallPut" + str(pSKStock.bstrCallPut) + 
               " 開盤價" + str(pSKStock.nOpen) + 
               " 最高價" + str(pSKStock.nHigh) + 
               " 最低價" + str(pSKStock.nLow) + 
               " 成交價" + str(pSKStock.nClose) + 
               " 結算價" + str(pSKStock.nSettlePrice) + 
               " 單量" + str(pSKStock.nTickQty) + 
               " 昨收、參考價" + str(pSKStock.nRef) + 
               " 買價" + str(pSKStock.nBid) + 
               " 買量" + str(pSKStock.nBc) + 
               " 賣價" + str(pSKStock.nAsk) + 
               " 賣量" + str(pSKStock.nAc) + 
               " 成交量" + str(pSKStock.nTQty) + 
               " 履約價" + str(pSKStock.nStrikePrice) + 
    ...
    def OnNotifyTicksNineDigitLONG(self, nIndex, nPtr, nDate, nTime, nClose, nQty):
        ...
        pSKTick, nCode = m_pSKOSQuote.SKOSQuoteLib_GetTickNineDigitLONG(nIndex, nPtr, pSKTick)
    ...
    def OnNotifyBest5NineDigitLONG(self, nStockidx, nBestBid1, nBestBidQty1, nBestBid2, nBestBidQty2, nBestBid3, nBestBidQty3, nBestBid4, nBestBidQty4, nBestBid5, nBestBidQty5, nBestAsk1, nBestAskQty1, nBestAsk2, nBestAskQty2, nBestAsk3, nBestAskQty3, nBestAsk4, nBestAskQty4, nBestAsk5, nBestAskQty5):
        ...
        pSKBest5, nCode = m_pSKOSQuote.SKOSQuoteLib_GetBest5NineDigitLONG(nStockidx, pSKBest5)
    ...
    def OnKLineData(self, bstrStockNo, bstrData):
        msg = "【OnKLineData】" + " 商品代號" + bstrStockNo + "_" + bstrData
        richTextBoxMessage.insert('end', msg + "\n")
        richTextBoxMessage.see('end')
SKOSQuoteEvent = SKOSQuoteLibEvent()
SKOSQuoteLibEventHandler = comtypes.client.GetEvents(m_pSKOSQuote, SKOSQuoteEvent)
...
class ConnectForm(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.buttonSKOSQuoteLib_EnterMonitorLONG = tk.Button(self)
        self.buttonSKOSQuoteLib_EnterMonitorLONG["text"] = "連線報價主機"
        self.buttonSKOSQuoteLib_EnterMonitorLONG["command"] = self.buttonSKOSQuoteLib_EnterMonitorLONG_Click
        self.buttonSKOSQuoteLib_EnterMonitorLONG.grid(row=0, column=1)
                
        # buttonSKOSQuoteLib_LeaveMonitor
        self.buttonSKOSQuoteLib_LeaveMonitor = tk.Button(self)
        self.buttonSKOSQuoteLib_LeaveMonitor["text"] = "斷線報價主機"
        self.buttonSKOSQuoteLib_LeaveMonitor["command"] = self.buttonSKOSQuoteLib_LeaveMonitor_Click
        self.buttonSKOSQuoteLib_LeaveMonitor.grid(row=1, column=1)
                        
        # buttonSKOSQuoteLib_IsConnected
        self.buttonSKOSQuoteLib_IsConnected = tk.Button(self)
        self.buttonSKOSQuoteLib_IsConnected["text"] = "檢查連線狀態"
        self.buttonSKOSQuoteLib_IsConnected["command"] = self.buttonSKOSQuoteLib_IsConnected_Click
        self.buttonSKOSQuoteLib_IsConnected.grid(row=2, column=1)
                
        # buttonSKOSQuoteLib_GetQuoteStatus
        self.buttonSKOSQuoteLib_GetQuoteStatus = tk.Button(self)
        self.buttonSKOSQuoteLib_GetQuoteStatus["text"] = "連線數資訊/限制"
        self.buttonSKOSQuoteLib_GetQuoteStatus["command"] = self.buttonSKOSQuoteLib_GetQuoteStatus_Click
        self.buttonSKOSQuoteLib_GetQuoteStatus.grid(row=3, column=1)
                
        # buttonSKOSQuoteLib_SetOSQuoteServer
        self.buttonSKOSQuoteLib_SetOSQuoteServer = tk.Button(self)
        self.buttonSKOSQuoteLib_SetOSQuoteServer["text"] = "切換資訊源"
        self.buttonSKOSQuoteLib_SetOSQuoteServer["command"] = self.buttonSKOSQuoteLib_SetOSQuoteServer_Click
        self.buttonSKOSQuoteLib_SetOSQuoteServer.grid(row=4, column=1)
                
        # buttonSKOSQuoteLib_Initialize
        self.buttonSKOSQuoteLib_Initialize = tk.Button(self)
        self.buttonSKOSQuoteLib_Initialize["text"] = "重新初始海期物件"
        self.buttonSKOSQuoteLib_Initialize["command"] = self.buttonSKOSQuoteLib_Initialize_Click
        self.buttonSKOSQuoteLib_Initialize.grid(row=5, column=1)

    def buttonSKOSQuoteLib_EnterMonitorLONG_Click(self):
        # 與報價伺服器連線
        nCode= m_pSKOSQuote.SKOSQuoteLib_EnterMonitorLONG()

        msg = "【SKOSQuoteLib_EnterMonitorLONG】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
        
    def buttonSKOSQuoteLib_LeaveMonitor_Click(self):
        
        # 中斷報價伺服器連線
        nCode= m_pSKOSQuote.SKOSQuoteLib_LeaveMonitor()

        msg = "【SKOSQuoteLib_LeaveMonitor】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
        
    def buttonSKOSQuoteLib_IsConnected_Click(self):
        
        # 檢查目前報價的連線狀態
        nCode= m_pSKOSQuote.SKOSQuoteLib_IsConnected()
        ...
        msg = "【SKOSQuoteLib_IsConnected】" + msg
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
        
    def buttonSKOSQuoteLib_GetQuoteStatus_Click(self):
        pnConnectionCount = 0
        pbIsOutLimit = False
        # 查詢報價連線狀態(是否超過報價連線限制,連線數資訊)
        nCode= m_pSKOSQuote.SKOSQuoteLib_GetQuoteStatus(pnConnectionCount, pbIsOutLimit)

        msg = "【SKOSQuoteLib_GetQuoteStatus】"  + "連線數:" + str(pnConnectionCount) + "超過限制:" + str(pbIsOutLimit)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
                
    def buttonSKOSQuoteLib_SetOSQuoteServer_Click(self):
        global sServer
        if sServer == 0:
            sServer = 1
        else:
        ...
        # 切換海期、海選報價資訊源
        nCode= m_pSKOSQuote.SKOSQuoteLib_SetOSQuoteServer(sServer)

        msg = "【SKOSQuoteLib_SetOSQuoteServer】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
        
    def buttonSKOSQuoteLib_Initialize_Click(self):
        # 重新初始海期物件
        nCode= m_pSKOSQuote.SKOSQuoteLib_Initialize()

        msg = "【SKOSQuoteLib_Initialize】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
######################################################################################################################################
#ProductForm
class ProductForm(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.buttonSKOSQuoteLib_RequestOverseaProducts = tk.Button(self)
        self.buttonSKOSQuoteLib_RequestOverseaProducts["text"] = "取得海外商品檔"
        self.buttonSKOSQuoteLib_RequestOverseaProducts["command"] = self.buttonSKOSQuoteLib_RequestOverseaProducts_Click
        self.buttonSKOSQuoteLib_RequestOverseaProducts.grid(row=0, column=1)
                
        # buttonSKOSQuoteLib_GetOverseaProductDetail
        self.buttonSKOSQuoteLib_GetOverseaProductDetail = tk.Button(self)
        self.buttonSKOSQuoteLib_GetOverseaProductDetail["text"] = "取得海期商品檔(含下單代碼)"
        self.buttonSKOSQuoteLib_GetOverseaProductDetail["command"] = self.buttonSKOSQuoteLib_GetOverseaProductDetail_Click
        self.buttonSKOSQuoteLib_GetOverseaProductDetail.grid(row=1, column=1)
        ...
        tk.Label(self, text = "商品代碼").grid(row=2, column=1)
            #輸入框
        self.textBoxSKOSQuoteLib_GetStockByNoLONG = tk.Entry(self)
        self.textBoxSKOSQuoteLib_GetStockByNoLONG.grid(row=2, column=2)

        global textBoxSKOSQuoteLib_GetStockByNoLONG
        textBoxSKOSQuoteLib_GetStockByNoLONG = self.textBoxSKOSQuoteLib_GetStockByNoLONG
                        
        # buttonSKOSQuoteLib_GetStockByNoLONG
        self.buttonSKOSQuoteLib_GetStockByNoLONG = tk.Button(self)
        self.buttonSKOSQuoteLib_GetStockByNoLONG["text"] = "個期資訊"
        self.buttonSKOSQuoteLib_GetStockByNoLONG["command"] = self.buttonSKOSQuoteLib_GetStockByNoLONG_Click
        self.buttonSKOSQuoteLib_GetStockByNoLONG.grid(row=2, column=3)
                                
        # buttonSKOSQuoteLib_GetStockByNoNineDigitLONG
        self.buttonSKOSQuoteLib_GetStockByNoNineDigitLONG = tk.Button(self)
        self.buttonSKOSQuoteLib_GetStockByNoNineDigitLONG["text"] = "個期資訊(CME九位擴充)"
        self.buttonSKOSQuoteLib_GetStockByNoNineDigitLONG["command"] = self.buttonSKOSQuoteLib_GetStockByNoNineDigitLONG_Click
        self.buttonSKOSQuoteLib_GetStockByNoNineDigitLONG.grid(row=2, column=4)
 

    def buttonSKOSQuoteLib_RequestOverseaProducts_Click(self):
        # 取得海外商品檔
        nCode= m_pSKOSQuote.SKOSQuoteLib_RequestOverseaProducts()

        msg = "【SKOSQuoteLib_RequestOverseaProducts】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
        
    def buttonSKOSQuoteLib_GetOverseaProductDetail_Click(self):
        sType = 1
        # 取得海期商品檔(含下單代碼)
        nCode= m_pSKOSQuote.SKOSQuoteLib_GetOverseaProductDetail(sType)

        msg = "【SKOSQuoteLib_GetOverseaProductDetail】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')

    def buttonSKOSQuoteLib_GetStockByNoLONG_Click(self):
        
        pSKStock = sk.SKFOREIGNLONG()

        # (LONG index)根據商品代號，取回海期報價的相關資訊。
        pSKStock, nCode= m_pSKOSQuote.SKOSQuoteLib_GetStockByNoLONG(textBoxSKOSQuoteLib_GetStockByNoLONG.get(), pSKStock)

        msg = "【SKOSQuoteLib_GetStockByNoLONG】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')

        msg = "【SKOSQuoteLib_GetStockByNoLONG】" + "商品代號:" + str(pSKStock.bstrStockNo) + " 商品名稱:" + str(pSKStock.bstrStockName) + " 報價小數位數:" + str(pSKStock.sDecimal) + " 分母:" + str(pSKStock.nDenominator) + " 市場代碼:" + str(pSKStock.bstrMarketNo) + " 交易所代號:" + str(pSKStock.bstrExchangeNo) + " 交易所名稱:" + str(pSKStock.bstrExchangeName) + " CallPut:" + str(pSKStock.bstrCallPut) + " 開盤價:" +  str(pSKStock.nOpen) + " 最高:" + str(pSKStock.nHigh) + " 最低:" + str(pSKStock.nLow) + " 成交價:" +  str(pSKStock.nClose) + " 結算價:" + str(pSKStock.nSettlePrice) + " 單量:" + str(pSKStock.nTickQty) + " 昨收(參考價):" +  str(pSKStock.nRef) + " 買價:" +  str(pSKStock.nBid) + " 買量:" + str(pSKStock.nBc) + " 賣價:" + str(pSKStock.nAsk) + " 賣量:" + str(pSKStock.nAc) + " 交易日:" + str(pSKStock.nTradingDay)
        richTextBoxMessage.insert('end', msg + "\n")
        richTextBoxMessage.see('end')
        
    def buttonSKOSQuoteLib_GetStockByNoNineDigitLONG_Click(self):
        
        pSKStock = sk.SKFOREIGN_9LONG()

        # (LONG index)(CME九位小數擴充)根據商品代號，取回海期報價的相關資訊。
        pSKStock, nCode= m_pSKOSQuote.SKOSQuoteLib_GetStockByNoNineDigitLONG(textBoxSKOSQuoteLib_GetStockByNoLONG.get(), pSKStock)

        msg = "【SKOSQuoteLib_GetStockByNoNineDigitLONG】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')

        msg = "【SKOSQuoteLib_GetStockByNoNineDigitLONG】" + "商品代號:" + str(pSKStock.bstrStockNo) + " 商品名稱:" + str(pSKStock.bstrStockName) + " 報價小數位數:" + str(pSKStock.sDecimal) + " 分母:" + str(pSKStock.nDenominator) + " 市場代碼:" + str(pSKStock.bstrMarketNo) + " 交易所代號:" + str(pSKStock.bstrExchangeNo) + " 交易所名稱:" + str(pSKStock.bstrExchangeName) + " CallPut:" + str(pSKStock.bstrCallPut) + " 開盤價:" +  str(pSKStock.nOpen) + " 最高:" + str(pSKStock.nHigh) + " 最低:" + str(pSKStock.nLow) + " 成交價:" +  str(pSKStock.nClose) + " 結算價:" + str(pSKStock.nSettlePrice) + " 單量:" + str(pSKStock.nTickQty) + " 昨收(參考價):" +  str(pSKStock.nRef) + " 買價:" +  str(pSKStock.nBid) + " 買量:" + str(pSKStock.nBc) + " 賣價:" + str(pSKStock.nAsk) + " 賣量:" + str(pSKStock.nAc) + " 交易日:" + str(pSKStock.nTradingDay)
        richTextBoxMessage.see('end')
######################################################################################################################################
#RequestForm
class RequestForm(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.buttonSKOSQuoteLib_RequestStocks = tk.Button(self)
        self.buttonSKOSQuoteLib_RequestStocks["text"] = "訂閱"
        self.buttonSKOSQuoteLib_RequestStocks["command"] = self.buttonSKOSQuoteLib_RequestStocks_Click
        self.buttonSKOSQuoteLib_RequestStocks.grid(row=0, column=5)
    ...
    def buttonSKOSQuoteLib_RequestStocks_Click(self):
        psPageNo = int(textBoxpsPageNo2.get())
        # 訂閱指定商品即時報價，要求伺服器針對 bstrStockNos 內的商品代號做報價通知動作。報價更新由OnNotifyQuote事件取得更通知。
        psPageNo, nCode= m_pSKOSQuote.SKOSQuoteLib_RequestStocks(psPageNo, textBoxStockNos.get())

        msg = "【SKOSQuoteLib_RequestStocks】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
######################################################################################################################################
#TicksForm
class TicksForm(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.buttonSKOSQuoteLib_RequestTicks = tk.Button(self)
        self.buttonSKOSQuoteLib_RequestTicks["text"] = "訂閱"
        self.buttonSKOSQuoteLib_RequestTicks["command"] = self.buttonSKOSQuoteLib_RequestTicks_Click
        self.buttonSKOSQuoteLib_RequestTicks.grid(row=0, column=5)
...
#十檔
    ...
    def buttonSKOSQuoteLib_RequestTicks_Click(self):
        psPageNo = int(textBoxpsPageNo.get())
        # 訂閱與要求傳送成交明細以及五檔
        psPageNo, nCode= m_pSKOSQuote.SKOSQuoteLib_RequestTicks(psPageNo, textBoxTicks.get())

        msg = "【SKOSQuoteLib_RequestTicks】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
######################################################################################################################################
#KLineForm
class KLineForm(tk.Frame):
    ...
    def createWidgets(self):
        ...
        # comboBoxsKLineType
        tk.Label(self, text = "K線種類").grid(row=1, column=1)
            #輸入框
        self.comboBoxsKLineType = ttk.Combobox(self, state='readonly')
        self.comboBoxsKLineType['values'] = Config.comboBoxsKLineType
        self.comboBoxsKLineType.grid(row=1, column=2)

        global comboBoxsKLineType
        comboBoxsKLineType = self.comboBoxsKLineType

        # textBoxbstrStartDate
        tk.Label(self, text = "起始日期").grid(row=2, column=1)
            #輸入框
        self.textBoxbstrStartDate = tk.Entry(self)
        self.textBoxbstrStartDate.grid(row=2, column=2)

        global textBoxbstrStartDate
        textBoxbstrStartDate = self.textBoxbstrStartDate
                       
        # textBoxbstrEndDate
        tk.Label(self, text = "結束日期").grid(row=3, column=1)
            #輸入框
        self.textBoxbstrEndDate = tk.Entry(self)
        self.textBoxbstrEndDate.grid(row=3, column=2)

        global textBoxbstrEndDate
        textBoxbstrEndDate = self.textBoxbstrEndDate
                       
        # textBoxsMinuteNumber
        tk.Label(self, text = "幾分K(分線)").grid(row=4, column=1)
            #輸入框
        self.textBoxsMinuteNumber = tk.Entry(self)
        self.textBoxsMinuteNumber.grid(row=4, column=2)

        global textBoxsMinuteNumber
        textBoxsMinuteNumber = self.textBoxsMinuteNumber
                        
        # buttonSKOSQuoteLib_RequestKLineByDate
        self.buttonSKOSQuoteLib_RequestKLineByDate = tk.Button(self)
        self.buttonSKOSQuoteLib_RequestKLineByDate["text"] = "KLine"
        self.buttonSKOSQuoteLib_RequestKLineByDate["command"] = self.buttonSKOSQuoteLib_RequestKLineByDate_Click
        self.buttonSKOSQuoteLib_RequestKLineByDate.grid(row=5, column=2)

    def buttonSKOSQuoteLib_RequestKLineByDate_Click(self):
        ...
        nCode= m_pSKOSQuote.SKOSQuoteLib_RequestKLineByDate(textBoxbstrStockNo.get(), sKLineType, textBoxbstrStartDate.get(), textBoxbstrEndDate.get(), int(textBoxsMinuteNumber.get()))

        msg = "【SKOSQuoteLib_RequestKLineByDate】" + m_pSKCenter.SKCenterLib_GetReturnCodeMessage(nCode)
        richTextBoxMethodMessage.insert('end',  msg + "\n")
        richTextBoxMethodMessage.see('end')
#==========================================
#定義彈出視窗
...
if __name__ == '__main__':
    ...
    # 開啟Product視窗的按鈕
    popup_button_Product = tk.Button(root, text="商品清單&個期資訊", command=popup_window_Product)
    ...
    # 開啟KLine視窗的按鈕
    popup_button_KLine = tk.Button(root, text="技術分析(KLine)", command=popup_window_KLine)
    popup_button_KLine.grid(row = 5, column= 0)
